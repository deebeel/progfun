...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[Neighbors] = List((left,Left),(right,Right),(down,Down),(up,Up))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[Neighbors] = neighbors filter {case (b,m) =>  b.isLegal}
